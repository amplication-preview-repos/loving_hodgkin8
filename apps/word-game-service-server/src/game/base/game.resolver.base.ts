...
export class GameResolverBase {
  ...
  @graphql.ResolveField(() => [Guess], { name: "guesses" })
  async findGuesses(
    @graphql.Parent() parent: Game,
    @graphql.Args() args: GuessFindManyArgs
  ): Promise<Guess[]> {
    const results = await this.service.findGuesses(parent.id, args);
  ...
  async getUser(@graphql.Parent() parent: Game): Promise<User | null> {
